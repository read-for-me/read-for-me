...
@router.post(
    ...
    summary="URL 크롤링",
    responses={
        400: {"model": CrawlErrorResponse, "description": "잘못된 URL 형식 또는 빈 입력"},
...
async def crawl_url(request: CrawlRequest) -> CleanedArticle:
    ...
    try:
        ...
        raise_crawl_error(UnsupportedContentError(
            url=url,
            domain=e.domain,
            reason=e.reason,
        ))
...
class UrlSupportResponse(BaseModel):
    ...
    platform: Optional[str] = Field(None, description="플랫폼 이름 (geeknews, medium, generic)")
    is_specialized: bool = Field(False, description="전용 크롤러 사용 여부")
    domain: str = Field("", description="파싱된 도메인")
    error_code: Optional[str] = Field(None, description="지원하지 않는 경우 에러 코드")
    error_message: Optional[str] = Field(None, description="지원하지 않는 경우 에러 메시지")
