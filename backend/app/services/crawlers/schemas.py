...
class ArticleMetadata(BaseModel):
    ...
    # Open Graph 태그
    og_title: str | None = Field(None, description="OG 제목")
    og_description: str | None = Field(None, description="OG 설명")
    og_image: str | None = Field(None, description="OG 이미지 URL")
    og_url: str | None = Field(None, description="OG URL")

    # 작성자 및 게시 정보
    author: str | None = Field(None, description="작성자")
    published_at: str | None = Field(None, description="게시일")

    # 플랫폼별 확장 필드
    original_url: str | None = Field(None, description="원본 외부 링크 (GeekNews)")
    points: str | None = Field(None, description="포인트 (GeekNews)")
    comment_count: int | None = Field(None, description="댓글 수")
    read_time: str | None = Field(None, description="읽는 시간 (Medium)")
    claps: str | None = Field(None, description="박수 수 (Medium)")
    tags: list[str] | None = Field(None, description="태그 목록")
    topic_id: str | None = Field(None, description="토픽 ID (GeekNews)")
    subtitle: str | None = Field(None, description="부제목 (Medium)")
...
class CrawledArticle(BaseModel):
    ...
    crawled_at: datetime = Field(
        default_factory=datetime.now, description="크롤링 시각"
    )
    metadata: ArticleMetadata = Field(
        default_factory=ArticleMetadata, description="메타데이터"
    )
    secondary_urls: list[str] = Field(
        default_factory=list, description="2차 URL 목록 (추후 확장용)"
    )

    # 원본 외부 링크의 크롤링된 콘텐츠 (GeekNews 등)
    # GEEKNEWS_CRAWL_ORIGINAL=true일 때 채워짐, 아니면 빈 문자열
    original_content: str = Field(
        default="", description="원본 외부 링크의 크롤링된 콘텐츠"
    )
...
class CleanedArticle(BaseModel):
    ...
    original_content: str = Field(
        default="", description="원본 외부 링크의 정제된 콘텐츠"
    )
